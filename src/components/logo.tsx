...
const LogoImage = ({ src, alt, className }: LogoImageProps) => (
  <Image src={src} alt={alt} width={120} height={32} className={cn("block h-8 w-auto", className)} />
);
...
const LogoText = ({ children, className, ...props }: LogoTextProps) => (
  <span
    className={cn("type-body-lg font-[var(--weight-semibold)] tracking-tighter", className)}
    {...props}
  >
    {children}
  </span>
...
const LogoTextMobile = ({ children, className, ...props }: LogoTextProps) => (
  <span
    className={cn(
      "type-body-lg font-[var(--weight-semibold)] tracking-tighter md:hidden",
      className,
    )}
    {...props}
  >
...
const LogoTextDesktop = ({ children, className, ...props }: LogoTextProps) => (
  <span
    className={cn(
      "hidden type-body-lg font-[var(--weight-semibold)] tracking-tighter md:flex",
      className,
    )}
    {...props}
  >
